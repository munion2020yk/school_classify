...
# 2. 사용자 입력 (슬라이더 UI - 숫자 직접 입력도 가능)
col1, col2 = st.columns(2)
with col1:
    a = st.slider("기울기 (a)", min_value=-5.0, max_value=5.0, value=0.0, step=0.1)
with col2:
    b = st.slider("y절편 (b)", min_value=-150, max_value=150, value=0, step=5)
...
# 그룹 1은 선 위(y > ax + b)에 있어야 함
for x, y in zip(group1_x, group1_y):
    if y <= a * x + b:
        success = False

# 그룹 2는 선 아래(y < ax + b)에 있어야 함
...
# 6. 결과 피드백 출력
if success:
    st.success("🎉 분류 성공! 하나의 선으로 두 그룹을 완벽하게 나누었습니다.")
    st.balloons()
else:
